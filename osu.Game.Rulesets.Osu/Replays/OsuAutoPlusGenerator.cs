...
namespace osu.Game.Rulesets.Osu.Replays
{
    public class OsuAutoPlusGenerator : OsuAutoPlusGeneratorBase
    {
        ...
        public override Replay Generate()
        {
            ...
            {
                addHitObjectReplay(Beatmap.HitObjects[i]);
            }
        ...
        private void addHitObjectReplay(OsuHitObject h)
        {
            ...
            {
                // if the spinner duration is shorter than time required for a full spin - don't bother.
                double spinnerDuration = spinner.EndTime - spinner.StartTime;
                double oneRotationTime = 2 * MathF.PI / 0.05;
                if (spinnerDuration < oneRotationTime)
                    return;
        ...
        private void addHitObjectClickFrames(OsuHitObject h, Vector2 startPosition)
        {
            ...
            // If the previous frame has a button pressed, add something in between so that it can actually count as a click.
            // We'll deal with alternating later. Hopefully.
            ...
            if (index >= 0)
            {
                ...
                // If a button is already held, then add an empty frame to force another click
                if (previousActions.Any())
                {
                    ...
                    // After removing the input we need to assign an action for the future.
            ...
            switch (h)
            {
                ...
                case Spinner spinner:
                    ...
                    {
                        t = ApplyModsToTimeDelta(previousFrame, nextFrame);
                    ...
                    t = ApplyModsToTimeDelta(previousFrame, endTime);
                    angle += (float)t * radsPerMillisecond;

                    Vector2 endPosition = SPINNER_CENTRE + CirclePosition(angle, SPIN_RADIUS);

                    AddFrameToReplay(new OsuReplayFrame(endTime, new Vector2(SPINNER_CENTRE.X, SPINNER_CENTRE.Y)));
            ...
            if (Frames[^1].Time <= endFrame.Time && h is not Spinner)
                AddFrameToReplay(endFrame);
        }

        private static void calcSpinnerStartPosAndDirection(Vector2 prevPos, out Vector2 startPosition)
        {
